...
class ConnectionDiagnostics(
    ...
    suspend fun <T> measure(write: Boolean,operation: String="request",authenticated: Boolean=true,request: suspend ()->T): T {
        ...
        try {
            val result=request()
            mutable.update { if(it.epoch!=epoch || latest[operation]!=requestId) it else it.copy(failures=it.failures-operation,failure=(it.failures-operation).values.firstOrNull(),serverReachable=true,sessionVerified=if(authenticated) true else it.sessionVerified,
            ...
            if(e !is CancellationException || e is TimeoutCancellationException) {
                mutable.update { if(it.epoch!=epoch || latest[operation]!=requestId) it else it.copy(
...
fun connectionFailure(e: Exception): String = when {
    e is SessionUnavailableException -> "session"
    e is SecurityException -> "permission"
    e.message=="location_disabled" -> "location"
    e.message=="location_permission" -> "permission"
