...
data class Snapshot(val profile: UserProfile? = null, val names: Map<String,String> = emptyMap(),
    ...
    val groupRequests: List<GroupRequest> = emptyList(),val meetings: List<MeetingPoint> = emptyList(),val syncFailed: Boolean=false,val realtimeUnavailable: Boolean=false,val savedPeople: Set<String> = emptySet(),val sharedPrecisionAvailable: Boolean=false,val locationRequestsAvailable: Boolean=false,val locationRequests: List<LocationRequest> = emptyList(),val eventsAvailable: Boolean=false,val events: List<AppEvent> = emptyList(),val temporaryGroupsAvailable: Boolean=false,val placesAvailable: Boolean=false,val sosAvailable: Boolean=false,val nearbySosAvailable: Boolean=false,val syncInProgress: Boolean=false,val syncError: String?=null)
