...
class EventNotificationWorker(context: Context,params: WorkerParameters): CoroutineWorker(context,params) {
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            val event=entry.client().postgrest.rpc("event_inbox").decodeList<AppEvent>().firstOrNull {it.id==id && it.kind in setOf("checkin","place","sos","sos_closed")} ?: return Result.success()
            ...
            val channel=if(event.kind=="sos") "sos-events" else "app-events"
            manager.createNotificationChannel(NotificationChannel(channel,context.getString(if(event.kind=="sos") R.string.sos_channel else R.string.checkin_channel),if(event.kind=="sos") NotificationManager.IMPORTANCE_HIGH else NotificationManager.IMPORTANCE_DEFAULT))
